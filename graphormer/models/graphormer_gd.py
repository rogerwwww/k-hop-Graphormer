...
class GraphormerGDEncoderBridge(FairseqEncoder):
    def __init__(self, args):
        super().__init__(dictionary=None)
        self.max_nodes = args.max_nodes
        ...
        self.activation_fn = utils.get_activation_fn(args.activation_fn)
        self.layer_norm = LayerNorm(args.encoder_embed_dim * 2)
    ...
    def forward(self, batched_data, perturb=None, masked_tokens=None, **unused):
        ...
        # zero padding
        padding_mask = (batched_data['x'] == 0).all(dim=-1).unsqueeze(-1)
        x_nocls = x[:, 1:, :].masked_fill(padding_mask, 0.0)
        # x_nocls = x[:, 1:, :]
        B, T, C = x_nocls.shape
        x_i = x_nocls[:, :, None, :].repeat(1, 1, T, 1)
        x_j = x_nocls[:, None, :, :].repeat(1, T, 1, 1)
        x = torch.cat((x_i, x_j), dim=-1).contiguous().view(B, -1, C * 2)
