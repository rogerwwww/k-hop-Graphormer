...
class GraphormerGDGraphEncoder(nn.Module):
    def __init__(
        ...
        # simulation:
        no_node_feature: bool = False,
        no_edge_feature: bool = False,
        num_rd_bias_kernel: int = 128,
        no_share_bias: bool = False,
        ...
        self.graph_node_feature = GraphNodeFeature(
            ...
            # @ simulation:
            no_node_feature=no_node_feature,
        )
        ...
        self.graph_attn_bias = GraphAttnBias(
            ...
            no_share=no_share_bias,
        )

        self.graph_rd_bias = GraphRDBias(
            num_heads=num_attention_heads,
            num_edges=num_edges,
            n_layers=num_encoder_layers,
            embed_dim=embedding_dim,
            num_kernel=num_rd_bias_kernel,
            no_share=no_share_bias,
        ...
        self.graph_attn_bias_mul = GraphAttnBias(
            num_heads=num_attention_heads,
            num_atoms=num_atoms,
            num_edges=num_edges,
            num_spatial=num_spatial,
            ...
            no_edge_feature=no_edge_feature,
            no_share=no_share_bias,
            no_edge_feature_mul=not mul_bias_with_edge_feature,
        )

        self.graph_rd_bias_mul = GraphRDBias(
            num_heads=num_attention_heads,
            num_edges=num_edges,
            n_layers=num_encoder_layers,
            embed_dim=embedding_dim,
    ...
    def build_graphormer_graph_encoder_layer(
        ...
        return GraphormerGDGraphEncoderLayer(
            embedding_dim=embedding_dim,
            ffn_embedding_dim=ffn_embedding_dim,
            num_attention_heads=num_attention_heads,
            dropout=dropout,
    ...
    def forward(
        ...
        attn_bias = self.graph_attn_bias(batched_data)
        attn_bias_mul = self.graph_attn_bias_mul(batched_data)

        attn_bias_rd, _ = self.graph_rd_bias(batched_data)
        attn_bias_rd_mul, _ = self.graph_rd_bias_mul(batched_data)
        attn_bias[:, :, 1:, 1:] = attn_bias[:, :, 1:, 1:] + attn_bias_rd
        attn_bias_mul[:, :, 1:, 1:] = attn_bias_mul[:, :, 1:, 1:] + attn_bias_rd_mul

        if self.no_share_bias:
            B, nh, T, _ = attn_bias.shape
            n_layer = len(self.layers)
            attn_bias = attn_bias.contiguous().view(B, n_layer, -1, T, T)
            attn_bias_mul = attn_bias_mul.contiguous().view(B, n_layer, -1, T, T)
        ...
        for i, layer in enumerate(self.layers):
            x, _ = layer(
                ...
                self_attn_padding_mask=padding_mask,
                self_attn_mask=attn_mask,
                self_attn_bias=attn_bias if not self.no_share_bias else attn_bias[:, i, :, :, :],
                self_attn_bias_mul=attn_bias_mul if not self.no_share_bias else attn_bias_mul[:, i, :, :, :]
            )
            if not last_state_only:
                inner_states.append(x)
